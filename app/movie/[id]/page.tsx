import Image from 'next/image';
import Link from 'next/link';
import { Play, Star, Clock, Calendar } from 'lucide-react';
import { Metadata } from 'next';
import { MovieDetail } from '@/lib/types';
import { tmdbImage } from '@/lib/constants';
import { getYear, formatRuntime } from '@/lib/utils';
import CastRow from '@/components/CastRow';
import Carousel from '@/components/Carousel';

export const dynamic = 'force-dynamic';

async function getMovie(id: string): Promise<MovieDetail | null> {
  try {
    const base = process.env.BACKEND_URL || process.env.NEXT_PUBLIC_API_URL || '';
    const res = await fetch(`${base}/api/movie/${id}`, { cache: 'no-store' });
    if (!res.ok) return null;
    return res.json();
  } catch {
    return null;
  }
}

export async function generateMetadata({ params }: { params: { id: string } }): Promise<Metadata> {
  const movie = await getMovie(params.id);
  if (!movie) return { title: 'Movie | StreamVault' };
  const year = getYear(movie.release_date);
  return {
    title: `${movie.title} (${year}) | StreamVault`,
    description: movie.overview,
    openGraph: {
      title: `${movie.title} (${year})`,
      description: movie.overview,
      images: movie.backdrop_path ? [tmdbImage(movie.backdrop_path, 'backdrop')] : [],
    },
  };
}

export default async function MoviePage({ params }: { params: { id: string } }) {
  const movie = await getMovie(params.id);

  if (!movie) {
    return (
      <div className="pt-32 px-8 text-center text-white/60">Movie not found.</div>
    );
  }

  return (
    <article className="relative">
      <div className="absolute inset-x-0 top-0 h-[70vh] -z-10">
        {movie.backdrop_path && (
          <Image
            src={tmdbImage(movie.backdrop_path, 'backdrop')}
            alt={movie.title}
            fill
            priority
            sizes="100vw"
            className="object-cover"
          />
        )}
        <div className="absolute inset-0 bg-black/60" />
        <div className="absolute inset-0 bg-gradient-to-t from-[#0a0a0a] via-[#0a0a0a]/70 to-transparent" />
      </div>

      <div className="pt-32 md:pt-40 px-4 md:px-8 max-w-[1400px] mx-auto">
        <div className="flex flex-col md:flex-row gap-6 md:gap-10">
          <div className="w-2/3 max-w-[240px] md:w-1/4 md:max-w-none flex-shrink-0">
            <div className="relative aspect-[2/3] rounded-md overflow-hidden shadow-2xl bg-[#1a1a1a]">
              <Image
                src={tmdbImage(movie.poster_path, 'poster')}
                alt={movie.title}
                fill
                sizes="(max-width: 768px) 60vw, 25vw"
                className="object-cover"
              />
            </div>
          </div>

          <div className="flex-1">
            <h1 className="text-3xl md:text-5xl font-extrabold">{movie.title}</h1>
            {movie.tagline && (
              <p className="text-white/60 italic mt-1">&ldquo;{movie.tagline}&rdquo;</p>
            )}

            <div className="flex flex-wrap items-center gap-3 mt-3 text-sm text-white/80">
              <span className="flex items-center gap-1">
                <Calendar className="w-4 h-4" /> {getYear(movie.release_date)}
              </span>
              {movie.runtime > 0 && (
                <>
                  <span>•</span>
                  <span className="flex items-center gap-1">
                    <Clock className="w-4 h-4" /> {formatRuntime(movie.runtime)}
                  </span>
                </>
              )}
              <span>•</span>
              <span className="flex items-center gap-1">
                <Star className="w-4 h-4 text-yellow-400" fill="currentColor" />{' '}
                {movie.vote_average?.toFixed(1)}
              </span>
              <span className="bg-white/15 rounded px-2 py-0.5 text-xs uppercase">HD</span>
            </div>

            <div className="flex flex-wrap gap-2 mt-4">
              {movie.genres?.map((g) => (
                <Link
                  href={`/genre/${g.id}`}
                  key={g.id}
                  className="bg-white/10 hover:bg-white/20 rounded-full px-3 py-1 text-sm transition-colors"
                >
                  {g.name}
                </Link>
              ))}
            </div>

            <p className="mt-5 text-white/85 leading-relaxed max-w-3xl">{movie.overview}</p>

            <div className="mt-6 flex flex-wrap gap-3">
              <Link
                href={`/watch/movie/${movie.id}`}
                className="bg-[#e50914] hover:bg-[#f40612] px-8 py-3 rounded font-semibold flex items-center gap-2 transition-colors"
              >
                <Play className="w-5 h-5" fill="white" /> Play Now
              </Link>
            </div>

            <CastRow cast={movie.credits?.cast || []} />
          </div>
        </div>

        <div className="mt-16 -mx-4 md:-mx-8">
          {movie.similar?.results?.length > 0 && (
            <Carousel
              title="Similar Titles"
              items={movie.similar.results.map((r) => ({ ...r, media_type: r.media_type || 'movie' }))}
            />
          )}
          {movie.recommendations?.results?.length > 0 && (
            <Carousel
              title="More Like This"
              items={movie.recommendations.results.map((r) => ({ ...r, media_type: r.media_type || 'movie' }))}
            />
          )}
        </div>
      </div>
    </article>
  );
}